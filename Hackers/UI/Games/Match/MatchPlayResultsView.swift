...
struct MatchPlayResultsView: View {
    ...
    private func compute() {
        ...
        playerData = ScoreUtil.Match.computeTotal(
        ...
        if !teams.isEmpty {
            teamData = ScoreUtil.Match.computeTotal(
                for: roundSession.players,
                over: session.holes,
                teams: true,
                skins: skins,
...
//        p.team = [1: "Team two", 2: "Team two", 3: "Team two", 4: "Team two"]
        ...
        s.score = [1: "triple", 2: "triple", 3: "triple", 4: "birdie"]
    ...
    static var previewSession: SideGameSession {
        return SideGameSession(
            id: "preview",
            game: SideGame.bestBall.rawValue,
            holes: [1, 2, 3, 4],
            stroke: nil,
            match: MatchSession(skins: true),
            monkey: nil,
            bingo: nil,
            chaos: nil,
            survivor: nil,
...
//        rs.teams = ["Team one", "Team two"]
    ...
    static var previews: some View {
        MatchPlayResultsView(session: previewSession)
